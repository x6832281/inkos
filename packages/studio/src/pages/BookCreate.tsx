...
export function BookCreate({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const sessionData = await fetchJson<{ session?: { sessionId?: string } }>("/sessions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ bookId: null }),
        });
        if (!cancelled && sessionData.session?.sessionId) {
          setAgentSessionId(sessionData.session.sessionId);
        }
      } catch {
        // ignore
      }
  ...
  const runAgentInstruction = async (instruction: string): Promise<AgentResponse> => {
    return fetchJson<AgentResponse>("/agent", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ instruction, sessionId: agentSessionId }),
    });
  };
          ...
          <div className="rounded-2xl border border-border/60 bg-card/70 p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                disabled={submitting || creating || !input.trim() || !agentSessionId}
              ...
              <button
                ...
                disabled={!canCreateFromDraft(draft) || creating || submitting || !agentSessionId}
